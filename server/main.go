...
func main() {
	http.HandleFunc("/solicitud/", HandleSolicitud)
...
func HandleSolicitud(w http.ResponseWriter, r *http.Request) {
	...
	id := strings.SplitN(strings.TrimPrefix(r.URL.Path, "/solicitud/"), "/", 2)[0]
	...
	htmlTemplate, err := os.ReadFile("server/views/solicitud.html")
	if err != nil {
		http.Error(w, "Failed to read template file", http.StatusInternalServerError)
		return
	}
	...
	if err != nil {
		http.Error(w, "Failed to render template", http.StatusInternalServerError)
		log.Printf("Template rendering error: %v", err) // Log the actual error
		return
	}
